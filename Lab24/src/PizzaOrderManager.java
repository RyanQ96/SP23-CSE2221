...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while(!fileIn.atEOS()) {
        	...
        	if (content.length() > 0) {
        		int idxOfSeperator = content.indexOf(","); 
        		String key = content.substring(0, idxOfSeperator); 
        		int value = Integer.parseInt(content.substring(idxOfSeperator+1));
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        // TODO - fill in body
        boolean gotOneOrder = false; 
        boolean gotSize = false; 
        int price = 0; 
        while (!input.atEOS() && !gotOneOrder) {
        	String content = input.nextLine();
        	if (content.length() == 0) {
        		gotOneOrder = true; 
        	} else {
        		if (!gotSize) {
        			price += sizePriceMap.value(content); 
        			gotSize = true; 
        		} else {
        			price += toppingPriceMap.value(content); 
        		}
        	}
        }
        /*
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        // TODO - fill in body
        int dollarAmount = price / 100; 
        int centAmount = price % 100; 
        output.print(dollarAmount + " dollars " + centAmount + " cents");

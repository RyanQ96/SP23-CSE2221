...
public final class Queue1LSort1 extends Queue1L<String> {
    ...
    private static String removeMin(Queue<String> q, Comparator<String> order) {
        ...
        // TODO - fill in body
        Queue<String> standBy = q.newInstance(); 
        String min = q.dequeue(); 
        while(q.length() > 0) {
        	String tmp = q.dequeue(); 
        	if (order.compare(min, tmp) > 0) {
        		standBy.enqueue(min); 
        		min = tmp; 
        	} else {
        		standBy.enqueue(tmp);
    ...
    public void sort(Comparator<String> order) {
        assert order != null : "Violation of: order is not null";
        Queue<String> res = this.newInstance(); 
        while (this.length() > 0) {
        	res.enqueue(removeMin(this, order));
        }
        this.transferFrom(res);
